...
const HandleMultiFileBannerImg = ({ ToggleModal, Images }) => {
  const [isUploading, setIsUploading] = useState();
  const dispatch = useDispatch();

  const uploadedFiles = useSelector((state) => state.UploadedImgs.banners);

  const onDrop = async (file) => {
    const fileName = `UI/product-banner/${nanoid(12)}-${file[0].name}`;
    ...
    try {
      ...
      const uploadImgOnServer = async () => {
        ...
        dispatch(uploadSingleImg(payload));
      };
...
export default React.memo(HandleMultiFileBannerImg);
